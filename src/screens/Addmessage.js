...
const onUpdateProfile=async(userid,msg)=>{
  
 try{
  setLoading(true);
  let response;
  
  response=await api.createMsg(userid,msg,'Unread');
...
<View style={styles.action}>
       ...
       <FontAwesome name="envelope-o" color={colors.text} size={20} /><Text style={{fontSize: 16, 
                     fontFamily: FONT_REGULAR,
                     marginLeft: 8,
