...
const EditAdminProfileScreen = (props) => {
...
const onUpdateProfile=async(userName,company,mobile,email,address,contact)=>{
  ...
  console.log(address);
  setLoading(true);
  let response=await api.updateProfile(props.route.params.id,userName,company,mobile,email,address,contact);
...
const onUpdateImage=async()=>{
  ...
  const callUserInfo=async(id,usertype)=>{
   let response;

    if(usertype=='Admin')
    {
    response=await api.getAdminListSingle(id);
    }else
    {
      response=await api.getUserListSingle(id);
    }
   ...
   // var fresponse=response.replace('Tu','');
    ...
    if(response.body[0].Path=null)
    {
    setImage(response.body[0].Path);
    }  
  }else{
